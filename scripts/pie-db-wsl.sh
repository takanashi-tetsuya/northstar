...
test_schema="${XMPP_TEST_SCHEMA:-northstar_pie_it_$random_suffix}"

if [[ "$test_database" != "xmpp_test" ]]; then
  echo "refusing to run PIE tests outside the disposable xmpp_test database" >&2
  exit 2
fi
if [[ ! "$test_schema" =~ ^northstar_pie_it_[a-f0-9]{32}$ ]] ||
   (( ${#test_schema} > 63 )); then
...
cleanup() {
  ...
  if [[ "$created" == 1 ]]; then
    ...
    [[ "$remains" == "f" ]] || {
      echo "isolated PIE schema was not removed: $test_schema (exists=$remains)" >&2
      status=1
    }
  fi
  exit "$status"
