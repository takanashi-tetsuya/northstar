...
| ARCH-RESTORE | 硬中斷後的還原判定 | 已解決的工具缺口＋運維前置條件 | **已實作** | 獨立恢復命令以 fsync journal、資料庫 transaction XID 與同交易結果標記判定前進或補償；S3 還原另驗證新鍵的精確版本，遇到證據缺漏則保持連線 fence 關閉 | 還原前仍須停止所有 writer、保留 rollback 制品及受保護的備份／物件版本；完整目標環境演練是發佈門檻 |
...
| EXT-CLUSTER | 叢集、CLU-MUC、capacity ledger 和共用儲存尚缺完整環境驗收 | 僅缺運行證據 | **完整矩陣通過後才可關閉** | 隔離 VM 已完成兩節點簽名 Redis 控制面、版本化 MinIO、跨節點雙向直達訊息；全域 Redis 停機及僅 B 到 Redis 的短暫分區均拒絕不安全新綁定，恢復後重新送達；B 硬中斷後，租約阻擋立即重入，到期後恢復；一個非空物件的 S3→Local→S3 離線往返保留 SHA-256。MinIO 程序停機時公開下載回 503，重啟後同一物件可按原雜湊讀回。這是探索性結果，詳見 `LOCAL_VM_QUALIFICATION.md` | 在凍結版本上補齊資料庫／物件儲存分區、lease loss、SM race、混合版本、Redis failover、遷移硬中斷恢復、S3/MinIO 磁碟遺失與新主機還原、provider lifecycle，保存設定、原始日誌和 RPO/RTO。完成前多節點仍為 `Experimental` |
