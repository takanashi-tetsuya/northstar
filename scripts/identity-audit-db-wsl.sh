...
test_schema="northstar_identity_audit_it_$random_suffix"

if [[ "$test_database" != "xmpp_test" ]]; then
  echo "identity audit tests are restricted to the dedicated xmpp_test database" >&2
  exit 2
fi
if [[ ! "$test_schema" =~ ^northstar_identity_audit_it_[a-f0-9]{32}$ ]] ||
   (( ${#test_schema} > 63 )); then
  echo "refusing an unsafe identity audit test schema name" >&2
  exit 2
fi
...
test_name="identity_audit::tests::postgres_audit_reports_all_rows_and_leaves_the_database_unchanged"
...
if ! grep -Eq 'test result: ok\. 1 passed; 0 failed' <<<"$output"; then
  echo "expected exactly one ignored identity audit database test to execute" >&2
  exit 1
fi
...
if [[ "$remaining" != "0" ]]; then
  echo "isolated identity audit schema was not removed" >&2
  exit 1
fi
trap - EXIT
