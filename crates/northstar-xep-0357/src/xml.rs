//! Safe XML building, fragment validation, and text escaping primitives for XEP-0357.
...
mod tests {
    ...
    fn validates_element_names() {
        assert!(validate_qname("enable").is_ok());
    ...
    fn builds_xml_elements() {
        let el = XmlElement::namespaced("notification", "urn:xmpp:push:0")
            .child(XmlElement::new("item"));
        assert_eq!(
            el.finish(),
            "<notification xmlns='urn:xmpp:push:0'><item/></notification>"
        );
    }
}
