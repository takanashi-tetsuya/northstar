...
| OPS-BACKUP-COMPAT | 生产备份已 fail-closed，仍保留显式 development legacy 与明文 rollback/人工硬崩溃处置 | 兼容性＋运维取舍 | **生产默认已关闭；剩余项可继续收紧** | base Compose 与脚本默认强制 Ed25519、age、sequence/restore floor 和 file-backed 分权 URL；legacy 只有单一 `development-legacy` 开关并警告。backup/restore 都在 Unix-socket-only 临时 PostgreSQL 验证 dump，不再用生产角色 createdb。restore 在 rollback dump/connection fence 前以事务方式预检当前库：仅接受空/bootstrap、migration-`0113` prepare 或 repository-authenticated exact current 状态，未知、部分或含 manifest 外 `public` 关系的库原地拒绝；incoming payload 始终要求 exact current ledger/schema，普通失败补偿则通过同一 canonical auto resolver 恢复旧库。restore 使用四个独立注册并预先核验 PID 的数据库 backend：维护控制连接位于 `postgres`，协调、主替换与补偿三个连接位于目标库。替换事务与 database-level outcome marker 原子提交，worker 断开不会再破坏控制/补偿通道或迫使脚本猜测结果。restore 不 terminate peer；发现三个已登记目标 PID 以外的连接时会暂时关闭新连接、拒绝 cutover，再重新开放未变更的目标库并要求先停服务。只有事务未结束、结果未知或补偿不完整时才保持 fail-closed。恢复前 rollback 目录仍可能是明文；控制 backend 自身突然死亡、SIGKILL/断电后的 journal/marker 仍需人工判定与恢复，sequence/floor state 丢失仍会改变 lineage 或可信下限 | rollback 放在加密卷并完成密钥/状态离机副本与恢复演练；实现跨进程确定性 journal resume 后再关闭硬崩溃项。任何 `allow-generation-change` 都需独立审计；legacy 兼容期结束后删除显式开发入口 |
