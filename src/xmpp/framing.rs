...
const MAX_XML_ELEMENTS: usize = 16_384;
const MAX_XML_ATTRIBUTES_PER_ELEMENT: usize = 128;
const MAX_XML_START_TAG_BYTES: usize = 64 * 1024;
const MAX_XML_DECLARATION_BYTES: usize = 1_024;

#[derive(Debug, thiserror::Error)]
enum XmlFramingError {
...
pub(crate) fn stream_error_condition(error: &anyhow::Error) -> &'static str {
...
fn restricted(feature: &'static str) -> anyhow::Error {
    XmlFramingError::Restricted(feature).into()
}

pub(crate) fn resource_limit(limit: &'static str) -> anyhow::Error {
    XmlFramingError::ResourceLimit(limit).into()
}

pub(crate) fn unsupported_encoding() -> anyhow::Error {
    XmlFramingError::UnsupportedEncoding.into()
}

fn reject_forbidden_xml_10_chars(xml: &str) -> Result<()> {
...
pub(crate) struct XmlEntityFramer {
...
impl XmlEntityFramer {
    ...
    pub(crate) fn take_frame(&mut self, buffer: &mut String) -> Result<Option<String>> {
    ...
    pub(crate) fn reset_pending_frame(&mut self) {
        self.scan.reset();
    }
}
...
fn validate_xml_declaration(declaration: &str) -> Result<()> {
    ...
    // Reuse the XML parser for quoting, duplicate-attribute and name syntax,
    // then enforce the small declaration grammar explicitly. XMPP streams are
    // UTF-8 and RFC 6120 is based on XML 1.0.
    let wrapper = format!("<declaration {content}/>");
    let document = roxmltree::Document::parse(&wrapper)
        .map_err(|_| anyhow::anyhow!("malformed XML declaration"))?;
    let root = document.root_element();
    let attributes = root.attributes().collect::<Vec<_>>();
    let ordered_names = attributes
        .iter()
    ...
    if !matches!(
        ...
        || attributes.iter().any(|attribute| {
            !matches!(attribute.name(), "version" | "encoding" | "standalone")
                || (attribute.name() == "standalone" && !matches!(attribute.value(), "yes" | "no"))
        })
    {
        bail!("unsupported or malformed XML declaration");
    }
    if attributes.iter().any(|attribute| {
        attribute.name() == "encoding" && !attribute.value().eq_ignore_ascii_case("UTF-8")
    }) {
