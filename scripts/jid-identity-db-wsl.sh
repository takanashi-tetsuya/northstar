...
test_schema="northstar_jid_identity_it_$random_suffix"

if [[ "$test_database" != "xmpp_test" ]]; then
  echo "JID identity tests are restricted to the dedicated xmpp_test database" >&2
  exit 2
fi
if [[ ! "$test_schema" =~ ^northstar_jid_identity_it_[a-f0-9]{32}$ ]] ||
   (( ${#test_schema} > 63 )); then
  echo "refusing an unsafe JID identity test schema name" >&2
  exit 2
fi
...
test_name="db::jid_identity::tests::postgres_migration_rewrites_exactly_rejects_collisions_and_is_idempotent"
...
if ! grep -Eq 'test result: ok\. 1 passed; 0 failed' <<<"$output"; then
  echo "expected exactly one ignored JID identity database test to execute" >&2
  exit 1
fi
...
if [[ "$remaining" != "0" ]]; then
  echo "isolated JID identity schema was not removed" >&2
  exit 1
fi
trap - EXIT
