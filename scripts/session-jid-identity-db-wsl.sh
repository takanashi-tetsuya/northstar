...
test_schema="northstar_session_jid_it_$random_suffix"

if [[ "$test_database" != "xmpp_test" ]]; then
  echo "session JID tests are restricted to the dedicated xmpp_test database" >&2
  exit 2
fi
if [[ ! "$test_schema" =~ ^northstar_session_jid_it_[a-f0-9]{32}$ ]] ||
   (( ${#test_schema} > 63 )); then
  echo "refusing an unsafe session JID test schema name" >&2
  exit 2
fi
...
test_name="db::session_identity::tests::postgres_session_identity_is_owned_atomic_resource_exact_and_idempotent"
...
if ! grep -Eq 'test result: ok\. 1 passed; 0 failed' <<<"$output"; then
  echo "expected exactly one ignored session JID database test to execute" >&2
  exit 1
fi
...
if [[ "$remaining" != "0" ]]; then
  echo "isolated session JID schema was not removed" >&2
  exit 1
fi
trap - EXIT
