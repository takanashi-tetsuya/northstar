...
/// Helper checking that node attributes are strictly within the allowed name list.
pub fn attributes_are(node: Node<'_, '_>, allowed: &[&str]) -> bool {
...
/// Helper checking that node text children contain only whitespace.
pub fn structural_text_is_empty(node: Node<'_, '_>) -> bool {
    node.children()
        .filter(|child| child.is_text())
        .all(|child| child.text().unwrap_or_default().trim().is_empty())
}

/// Helper checking that node text content has only whitespace.
pub fn has_only_whitespace_text(node: Node<'_, '_>) -> bool {
    node.children()
        .filter(|child| child.is_text())
...
mod tests {
    ...
    fn validates_element_names() {
        assert!(validate_qname("query").is_ok());
        assert!(validate_qname("x:data").is_ok());
    ...
    fn builds_xml_elements() {
        let el =
            XmlElement::namespaced("query", "urn:xmpp:mam:2").child(XmlElement::new("flip-page"));
        assert_eq!(
            el.finish(),
            "<query xmlns='urn:xmpp:mam:2'><flip-page/></query>"
        );
    }
}
