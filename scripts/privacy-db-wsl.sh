...
schema="northstar_privacy_it_${suffix}"

if [[ "$database" != "xmpp_test" ]] ||
   [[ ! "$schema" =~ ^northstar_privacy_it_[a-f0-9]{32}$ ]] ||
   (( ${#schema} > 63 )); then
  echo "refusing unsafe privacy-list test target" >&2
  exit 2
fi
...
cargo test --locked --offline \
  db::privacy::tests::postgres_privacy_lists_are_bounded_ordered_cascading_and_enforced \
  -- --ignored --exact --nocapture --test-threads=1
cargo test --locked --offline \
  services::privacy::tests::privacy_service_boundary_preserves_stanza_kind_allow_deny_results \
  -- --ignored --exact --nocapture --test-threads=1
...
remaining="$(psql "${db[@]}" --tuples-only --no-align \
  --command "SELECT EXISTS(SELECT 1 FROM pg_namespace WHERE nspname='$schema')")"
[[ "$remaining" == "f" ]] || { echo "privacy-list schema cleanup failed" >&2; exit 1; }
trap - EXIT
