...
| GET `/readyz` | 内部编排 | 检查 PostgreSQL、持久化安全 authority 与受监督 worker；默认 Caddy 对公网返回 404，重复探针共享短缓存与单飞限制 |
| GET `/metrics` | 独立私有 listener | Prometheus 文本；不经过公共 HTTP router，非 loopback 必须使用 bearer token |
...
| POST `/api/v1/admin/tls/reload` | admin bearer | 原子重读证书、信任根和 CRL，返回单调 TLS generation 与精确 drain 数；新握手使用新快照，只有新 CRL 对现有 C2S/S2S SASL EXTERNAL 链给出明确 `CertRevoked` 时才断开对应连接 |
...
这段 v0.1 快照当时记录的是启动自动迁移。当前版本已改为显式
`xmpp-server migrate`：只有一次性 migrator 身份可以变更 schema，常驻服务
只会唯读核对 migration ledger、checksum 与 RFC 7622 marker。生产变更仍应先
完成 PostgreSQL 备份；当前没有 down migration。
