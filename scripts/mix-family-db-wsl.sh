...
test_schema="${XMPP_TEST_SCHEMA:-northstar_mix_family_$random_suffix}"

if [[ "$test_database" != "xmpp_test" ]]; then
  echo "refusing to run MIX family tests outside the disposable xmpp_test database" >&2
  exit 2
fi
if [[ ! "$test_schema" =~ ^northstar_mix_family_[a-f0-9]{32}$ ]] ||
   (( ${#test_schema} > 63 )); then
...
cleanup() {
  ...
  if [[ "$created" == 1 ]]; then
    ...
    [[ "$remains" == "f" ]] || {
      echo "isolated MIX family schema was not removed: $test_schema (exists=$remains)" >&2
      status=1
    }
  fi
  exit "$status"
