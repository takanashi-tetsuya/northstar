...
| `deployment-capacity-session-leases` | `main` | critical / continuous | `2 ×` lease interval | immediate | renew the local process's routable session-capacity leases | terminal attempt/liveness failure cancels the service; an idle node performs no database work in this worker |
