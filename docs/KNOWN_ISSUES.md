...
| EXT-CLUSTER | 叢集、CLU-MUC、capacity ledger 和共用儲存尚缺完整環境驗收 | 僅缺運行證據 | **完整矩陣通過後才可關閉** | 隔離 VM 已完成兩節點簽名 Redis 控制面、版本化 MinIO、跨節點雙向直達訊息，以及 Redis 停機時拒絕新綁定、恢復後重新送達；S3 遷移只處理零物件。這是探索性結果，詳見 `LOCAL_VM_QUALIFICATION.md` | 在凍結版本上補齊非對稱分區、lease loss、SM race、混合版本、Redis failover、非空 S3 遷移與 crash/restore、provider lifecycle，保存設定、原始日誌和 RPO/RTO。完成前多節點仍為 `Experimental` |
