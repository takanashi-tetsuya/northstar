...
use crate::api::models::{
    BooleanToggle, BroadcastRequest, MucRoomView, NukeRequest, OfflineMessagesStats,
    ServerNamePatch, SessionView,
};
use crate::db;
...
pub async fn admin_rename_server(
    ...
    if env_path.exists() {
        ...
        if !found {
            new_content.push('\n');
            new_content.push_str(&format!("SERVER_NAME=\"{}\"", body.server_name));
            new_content.push('\n');
        }
    ...
    Ok(Json(
        json!({"server_name": body.server_name, "message": "Server name updated in .env (requires restart)"}),
    ))
}
...
pub async fn admin_panic_disconnect(
    ...
    Ok(Json(
        json!({"sessions_dropped": sessions_count, "message": "All users forced offline"}),
    ))
}
...
pub async fn admin_destroy_muc_room(
    ...
    if result.rows_affected() > 0 {
        ...
        state
            .muc_occupants
            .retain(|_, occ| occ.room_jid != room_jid);
...
pub async fn admin_broadcast(
    ...
    for entry in state.sessions.iter() {
        let session = entry.value();
        if session.sender.try_send(message.clone()).is_ok() {
            sent_count += 1;
        }
    }
