...
pub(crate) enum FederationDeliveryMode {
    DurableOutbox,
    Volatile,
}
...
impl FederationEnvelope {
    ...
    pub(crate) fn complete_volatile_delivery(&mut self) {
        if let Some(completion) = self.volatile_completion.take() {
            let _ = completion.send(());
        }
    }

    pub(crate) fn volatile_write_budget(&self) -> Option<std::time::Duration> {
        if self.is_durable() {
            return None;
        }
        if self
...
impl From<crate::db::S2sOutboxItem> for FederationEnvelope {
    fn from(item: crate::db::S2sOutboxItem) -> Self {
        Self {
            outbox_id: item.id,
