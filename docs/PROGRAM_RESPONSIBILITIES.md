...
| runtime control pool | `northstar_runtime` | exactly 1 reserved connection, 500 ms acquire bound | durable administration-settings refresh; XEP-0133 restart/shutdown watcher when enabled | process startup reserves and attests it before constructing the traffic pool; it remains separate from traffic and command pools, so an exhausted workload pool cannot hide committed safety configuration |
