...
if [[ "$test_database" != "xmpp_test" ]]; then
  echo "refusing to run abuse-key tests outside the disposable xmpp_test database" >&2
  exit 2
fi
if [[ ! "$test_schema" =~ ^northstar_abuse_key_[a-f0-9]{32}$ ]] ||
   (( ${#test_schema} > 63 )); then
...
cleanup() {
  ...
  if [[ "$created" == "1" ]]; then
    ...
    if [[ "$remains" != "f" ]]; then
      echo "isolated abuse-key schema was not removed: $test_schema (exists=$remains)" >&2
      status=1
    fi
  fi
  exit "$status"
