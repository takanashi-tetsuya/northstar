...
| `runtime-control-refresh` | `AppState` runtime-control startup | critical / continuous | 5 s | immediate | sequentially refresh durable federation rules and security-relevant administration settings each second, and observe XEP-0133 service control every 500 ms when enabled, through the one reserved control-plane connection | splitting control-plane reads into competing workers, silently retaining a superseded security setting, continuing with stale allow/deny authority, or keeping listeners available after committed shutdown control changes |
