...
> 基线：当前 `dev` 工作区；迁移 `0126`–`0137`、XEP-0198 事件通知、MIX durable route-wake、跨节点 typed transport hand-off 与 XEP-0115 observation 重构已经进入代码，但发布前仍须对最终提交重新保存完整门禁证据。`0130` 的持久个人消息 admission 索引/账号删除候选查找，以及 `0131`–`0137` 的数据库安全与有序投递修复都仍处于 M00 验证中，不能据此声称 production accepted。
...
| 项目 | 结果 | 证据范围 |
| --- | --- | --- |
| `0126`–`0137`、数据库权限清单、SM、Caps 与 WebSocket 终止序列确定性回归 | `0130`–`0137` 后的最终结果尚未记录；M00 仍为进行中，不能视为 production accepted | 先前记录的 `1,188 total / 1,021 passed / 167 ignored / 0 failed` 早于当前 `0130`–`0137` 变更，只能作为历史对照。必须在同一最终 tree 上重新执行普通 Rust、隔离 PostgreSQL/Redis 与发布 CI 门禁；ignored 不得视为通过 |
...
| 外发 XML 与文档/迁移门禁 | `0137` ledger 已由 repository generator 重建，静态 migration、role/capability manifest 门禁已执行；最终提交仍须在 CI 重跑 | 迁移当前最大值为 `0137`、总数为 136；每行由 SQLx description 与 SHA-384 绑定。实际应用迁移、角色授权与回滚仍由隔离 CI/目标环境证明 |
