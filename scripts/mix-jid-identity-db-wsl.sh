...
test_schema="northstar_mix_jid_it_$random_suffix"

if [[ "$test_database" != "xmpp_test" ]]; then
  echo "MIX JID tests are restricted to the dedicated xmpp_test database" >&2
  exit 2
fi
if [[ ! "$test_schema" =~ ^northstar_mix_jid_it_[a-f0-9]{32}$ ]] ||
   (( ${#test_schema} > 63 )); then
  echo "refusing an unsafe MIX JID test schema name" >&2
  exit 2
fi
...
if ! grep -Eq 'test result: ok\. 1 passed; 0 failed' <<<"$output"; then
  echo "expected exactly one ignored MIX JID database test to execute" >&2
  exit 1
fi
...
if [[ "$remaining" != "0" ]]; then
  echo "isolated MIX JID schema was not removed" >&2
  exit 1
fi
trap - EXIT
