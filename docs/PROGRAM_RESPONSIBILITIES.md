...
| `runtime-control-refresh` | `AppState` runtime-control startup | critical / continuous | 5 s | immediate | sequentially refresh the runtime projections of durable federation rules and security-relevant administration settings through the one reserved control-plane connection | splitting the two refreshes into competing workers, silently retaining a superseded security setting, or continuing with stale allow/deny authority |
