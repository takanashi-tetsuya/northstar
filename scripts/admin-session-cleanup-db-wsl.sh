...
if [[ "$test_database" != "xmpp_test" ]]; then
  echo "administrator cleanup tests are restricted to the dedicated xmpp_test database" >&2
  exit 2
fi
if [[ ! "$test_schema" =~ ^northstar_admin_cleanup_it_[a-f0-9]{32}$ ]] ||
   (( ${#test_schema} > 63 )); then
  echo "refusing an unsafe administrator cleanup schema name" >&2
  exit 2
fi
...
run_exact_ignored() {
  local test_name="$1"
  local output
  if ! output="$(cargo test --locked --offline "$test_name" -- \
      --ignored --exact --nocapture --test-threads=1 2>&1)"; then
    printf '%s\n' "$output"
    return 1
  fi
  printf '%s\n' "$output"
  grep -Eq 'test result: ok\. 1 passed; 0 failed' <<<"$output" || {
    echo "expected exactly one ignored administrator cleanup test: $test_name" >&2
    return 1
  }
}
...
if [[ "$remaining" != "0" ]]; then
  echo "isolated administrator cleanup schema was not removed" >&2
  exit 1
fi
trap - EXIT
