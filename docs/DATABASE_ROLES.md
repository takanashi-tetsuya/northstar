...
151 migrations from `0001` through `0152`; `0021` is the sole intentional gap.
...
requires the complete checked-in `0001`-`0152` manifest, not merely the
...
Migrations `0131` through `0142` are forward security/capability hardenings, not
...
routine bindings, owners, search paths and grants. `0139` repairs the
expired-upload cleanup admission's named conflict target while preserving its
existing function signature and schema-pinned `SECURITY DEFINER` boundary.
`0140` moves the two physical-projection delete accounting edges to `BEFORE
DELETE`, so only the final surviving projection can release a logical retained
object during a multi-row deletion; it retains their owner-only capability
shape and exact schema-pinned paths. `0141` reasserts the complete fixed-path,
`SECURITY DEFINER`, and `PUBLIC`-revoked contract for the existing expired
cleanup admission after its immutable `0139` replacement; it does not rewrite
the earlier migration or widen the runtime grant surface. `0142` applies the
...
the successful sqlx ledger with all 151 checked-in migrations from `0001`
through `0152` (including the intentional numbering gap at `0021`);
...
The `0001`-`0152` migration SQL and checksums used by both the one-shot migrator
...
Migration `0144` records account revocations in the credential-change transaction.
Each cluster process consumes its own queue and acknowledges the revision it read.
...
Migration `0148` admits durable cluster MUC administrative batches and the
existing subject operation. Roll out readers before enabling batch writers.
Migration `0149` adds the offline upload migration journal and restore outcome
markers. The migrator owns these tables; backup can read them for a complete
logical dump, while runtime, storage and command roles have no table access.
Runtime can execute only the owner-held `northstar_storage_migration_active()`
boolean probe; startup refuses an unfinished migration without reading its
journal.
Storage cutover requires stopped nodes and drained upload work. Restore writes
its outcome marker in the same transaction as database replacement.
Migration `0150` adds a narrow runtime capability to lock an enabled account's
current authentication generation inside a transaction. It avoids granting
...
Migration `0152` adds an index for MIX MAM's timestamp and authoritative stanza
ID order. It changes no workload grants.
