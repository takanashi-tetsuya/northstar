...
| `cluster_session_route_maintenance_service()` | periodic durable cluster session-route maintenance | bounded cleanup of 4096 routes followed by authority validation through the PostgreSQL repository | refreshing instance authority after a failed cleanup or validation; hiding route authority failure from fail-closed policy |
