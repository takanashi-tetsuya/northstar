...
| OPS-BACKUP-COMPAT | 回滾制品可能是明文，且保留 `development-legacy` 模式 | 維運取捨 | **仍需收緊** | 生產備份預設要求 Ed25519 簽名、age 加密及可信還原代際。還原使用停止寫入的窗口、私有 journal、資料庫 XID 與同交易標記；`recover-restore.sh` 可在 `SIGKILL` 後核驗證據並繼續或補償。證據衝突、過舊而無法判定的 XID，或缺失的物件版本都會保留連線 fence。回滾目錄預設仍可能保存明文 dump 與物件 | 將回滾目錄放在加密儲存，離機保存密鑰與可信狀態，並在目標環境演練硬中斷復原；過舊 XID 的不確定情況仍須人工處理。相容期結束後移除 `development-legacy` |
