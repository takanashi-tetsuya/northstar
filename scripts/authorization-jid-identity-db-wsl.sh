...
test_schema="northstar_auth_jid_it_$random_suffix"

if [[ "$test_database" != "xmpp_test" ]]; then
  echo "authorization JID tests are restricted to the dedicated xmpp_test database" >&2
  exit 2
fi
if [[ ! "$test_schema" =~ ^northstar_auth_jid_it_[a-f0-9]{32}$ ]] ||
   (( ${#test_schema} > 63 )); then
  echo "refusing an unsafe authorization JID test schema name" >&2
  exit 2
fi
...
test_name="db::authorization_identity::tests::postgres_batch_rewrites_rolls_back_collisions_and_is_idempotent"
...
if ! grep -Eq 'test result: ok\. 1 passed; 0 failed' <<<"$output"; then
  echo "expected exactly one ignored authorization JID database test to execute" >&2
  exit 1
fi
...
if [[ "$remaining" != "0" ]]; then
  echo "isolated authorization JID schema was not removed" >&2
  exit 1
fi
trap - EXIT
