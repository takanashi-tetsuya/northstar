...
if [[ "$database_transport" == 'private-unix-socket' ]]; then
  ...
  # The directory is generated by the wrapper above and the exact allow-list
  # prevents this destructive fixture from following a caller-supplied path.
  encoded_database_host="${database_host//\//%2F}"
  migrator_database_url="postgresql://northstar_migrator:${migrator_password}@/xmpp?host=${encoded_database_host}"
else
