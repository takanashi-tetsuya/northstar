...
本手册用于在授权的隔离环境中执行安全、故障恢复和极端负载验证。
每次执行须记录提交、环境与结果；部分矩阵也由定时或手动 CI 运行。
测试目标限于专用临时实例，不得使用生产服务、共享数据库或真实用户账号。
...
- **负载配置**：建立会话和重新认证最多使用 8 个并行任务；1,000 个已认证会话同时保留，消息接收与 ping 使用 64 个任务。SM 恢复使用 SASL2 装置标识，保持默认的同装置检查。SM 恢复及过载后的重连都先等待旧 TCP 连接关闭，清理等待上限为 30 秒；最终资源检查前最多等待 60 秒。
