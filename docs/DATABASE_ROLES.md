...
139 migrations from `0001` through `0140`; `0021` is the sole intentional gap.
...
requires the complete checked-in `0001`-`0140` manifest, not merely the
...
Migrations `0131` through `0140` are forward security/capability hardenings, not
...
shape and exact schema-pinned paths.
...
the successful sqlx ledger with all 139 checked-in migrations from `0001`
through `0140` (including the intentional numbering gap at `0021`);
...
The `0001`-`0140` migration SQL and checksums used by both the one-shot migrator
