...
> 基线：当前 `dev` 工作区；迁移 `0126`–`0132`、XEP-0198 事件通知与 XEP-0115 observation 重构已经进入代码，但发布前仍须对最终提交重新保存完整门禁证据。`0130` 的持久个人消息 admission 索引/账号删除候选查找，以及 `0131`/`0132` 的数据库安全修复都仍处于 M00 验证中，不能据此声称 production accepted。
...
| 项目 | 结果 | 证据范围 |
| --- | --- | --- |
| `0126`–`0132`、数据库权限清单、SM、Caps 与 WebSocket 终止序列确定性回归 | `0130`–`0132` 后的最终结果尚未记录；M00 仍为进行中，不能视为 production accepted | 先前记录的 `1,188 total / 1,021 passed / 167 ignored / 0 failed` 早于当前 `0130`–`0132` 变更，只能作为历史对照。必须在同一最终 tree 上重新执行普通 Rust、隔离 PostgreSQL/Redis 与发布 CI 门禁；ignored 不得视为通过 |
...
| 外发 XML 与文档/迁移门禁 | 当前 `0130`–`0132` 工作树尚待重新生成 ledger 并运行门禁 | 迁移当前最大值为 `0132`、总数为 131；必须以新 SQL 的 SHA-384 重建 repository ledger，并重新执行权限清单、授权脚本、现有卷审计与启动 attestation。实际应用迁移、角色授权与回滚仍由隔离 CI/目标环境证明 |
