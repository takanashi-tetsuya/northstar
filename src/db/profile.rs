//! PostgreSQL profile snapshots, avatar projection and durable publication.

use crate::{db, services::profile::*, xmpp::xml_builder::XmlElement};
...
impl ProfileRepository for PostgresProfileRepository {
    async fn public_vcard(&self, username: &str) -> Result<PublicVCard> {
        ...
        match row {
            Some(row) => Ok(PublicVCard::Profile(
                if row
                    .try_get::<Option<Uuid>, _>("profile_account_id")?
                    .is_some()
                {
                    Some(row.try_get("payload")?)
                } else {
                    None
                },
            )),
    ...
    async fn publish_profile_items(
        ...
        let outcome = db::pep::publish_pep_items_in_transaction(
            ...
            write.user_id,
            write.node,
            &db::PepNodeConfig::from(write.requested),
...
mod tests {
    ...
    impl ProfileService<PostgresProfileRepository> {
        fn new(pool: PgPool, domain: String) -> Self {
            let admission = Arc::new(crate::services::pubsub::PubSubMutationAdmission::new(
                pool.options().get_max_connections() as usize,
            ));
            Self::with_mutation_admission(PostgresProfileRepository::new(pool, domain), admission)
        }
    }
    ...
    async fn deleted_account_incarnation_and_corrupt_vcard_fail_closed() {
        ...
        assert_eq!(
            ...
            PublicVCard::Profile(None)
        );
        assert_eq!(
            service
                .public_vcard("missing-profile-account")
                .await
                .unwrap(),
        ...
        assert!(service.public_vcard(&username).await.is_err());
        let config = db::default_pep_node_config(AVATAR_METADATA).into();
        let items = [(
            "current",
            "<item id='current'><metadata xmlns='urn:xmpp:avatar:metadata'/></item>",
        )];
        assert!(service
            ...
            .await
            .is_err());
        let still_corrupt: bool =
            sqlx::query_scalar("SELECT payload IS NULL FROM vcards WHERE user_id=$1")
                .bind(new_id)
                .fetch_one(&database.pool)
                .await
                .unwrap();
        assert!(
            still_corrupt,
            "failed publication must not overwrite the damaged row"
        );
        database.finish().await;
    }
}
