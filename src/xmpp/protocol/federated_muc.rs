...
use crate::services::muc::{
    ...
    MucSubjectOutcome, OfflineStoreOutcome,
};
use crate::state::{
...
async fn unregister_remote_occupant(
    ...
    let local_departure_guard = if state.federated_muc_pg_authority_enabled() {
        ...
        let Some(initial_room) = state
            ...
            state.remove_local_muc_occupant_exact((&departed).into());
            return Ok(());
        };
        let guard = state
            .muc_service()
        ...
        let Some(current) = state.local_muc_occupant_exact((&departed).into()) else {
            return Ok(());
        };
        departed = current;
    ...
    let removed_globally = if clustered_leave {
        ...
        state
            .federated_muc_evict_occupant(&serializable, removal_status.unwrap_or(307), None, None)
            .await?
    };
    let globally_empty = removed_globally
        && state
            .federated_muc_global_occupants(&room_jid)
            .await?
            .is_empty();
    if locally_empty && globally_empty {
        state.federated_muc_leave_room(&room_jid).await?;
    }
    if !clustered_leave {
        state
            .federated_muc_publish_removal_presence(
                &room_jid,
                &serializable,
                removal_status,
                None,
...
async fn federated_muc_presence_owned(
    ...
    if matches!(
        ...
        if let Some((_, occupant)) = existing {
            ...
            unregister_remote_occupant(state, occupant, status).await?;
        }
        return Ok(None);
    }
    ...
    if let Some((_, mut occupant)) = existing {
        ...
        if occupant.nick != nick {
            ...
            state
                .federated_muc_publish_nickname_change(
                    &room_jid,
                    &old_serializable,
                    &new_serializable,
    ...
    if state.federated_muc_pg_authority_enabled() {
        ...
        match state
            ...
            .claim_local_cluster_occupancy(ClusterMucJoin {
                ...
                nick,
                owner_node_id: state.federated_muc_owner_node_id(),
    ...
    if publication != crate::state::LocalMucJoinPublication::Published {
        ...
        if publication == crate::state::LocalMucJoinPublication::Occupied {
            tracing::error!(room=%room_jid, %nick,
                "committed federated MUC join found another local incarnation; preserving it and compensating this claim");
            drop(local_join_guard);
            super::muc::compensate_unpublished_cluster_muc_join(state, &room, &occupant).await;
            return Ok(federated_error(&request.stanza, from, "cancel", "conflict"));
        }
    }
    drop(local_join_guard);
...
async fn federated_muc_message_owned(
    ...
    if let Some((target_raw, reason)) = decline {
            ...
            if !delivered && request.temporary_storage {
                ...
                let outcome = state
                    ...
                    .store_federated_muc_offline(
                        ...
                        &delayed,
                        false,
                        super::muc::temporary_muc_offline_policy(state),
                    )
                    .await?;
        ...
        if !state.xmpp_external_route_domain_allowed(target.domainpart())
            || !state
                .federation_outbox()
    ...
    if let Some(voice_form) = voice_form {
        ...
        match voice_form {
            super::muc::MucVoiceForm::Request => {
                ...
                {
                    if let Some(local) = state.local_muc_occupant_exact(moderator.into()) {
                        let _ = state
                    ...
                    state
                        .federated_muc_publish_private_message(
                            &room_jid,
                            &moderator.nick,
                            &voice_request,
                            &own.full_jid,
            ...
            super::muc::MucVoiceForm::Approval { jid, nick, allow } => {
                ...
                if !state.federated_muc_pg_authority_enabled() {
                    ...
                    let Some(current_actor) = state.local_muc_occupant_exact((&own).into()) else {
                        return Ok(federated_error(&request.stanza, from, "auth", "forbidden"));
                    };
    ...
    if to.contains('/') {
        ...
        let local_target = state.local_muc_occupant_by_nick(&room_jid, target_nick);
        let (target_full_jid, route_via_cluster) = if let Some(target) = &local_target {
            (target.full_jid.clone(), false)
        } else {
            let global = state.federated_muc_global_occupants(&room_jid).await?;
                    ...
                    .await
                {
                    Ok(true) => {
                        ...
                        if cluster_authority.is_some() {
                            state.wake_committed_muc_operation(operation_id).await?;
                        }
                    }
                    Ok(false) => {
                        return Ok(federated_error(&request.stanza, from, "auth", "forbidden"));
    ...
    let archive_enabled = room.logging_enabled
        ...
        && (encrypted || !state.archive_requires_encryption());
    ...
    if let Some(target_id) = author_retraction {
        ...
        let admission = state
            ...
            .admit_local_discussion(MucDiscussion {
                ...
                retention_days: state.muc_mam_retention_days(),
                authority: actor_authority,
            })
            .await?;
        match admission {
    ...
    match state
        .federated_muc_publish_room_message(&room_jid, &reflected, Some(&actor_full_jid))
        .await
    {
        Ok(()) => {}
...
async fn federated_muc_iq_owned(
                ...
                let mut query =
                    XmlElement::namespaced("query", "http://jabber.org/protocol/disco#info").child(
                        XmlElement::new("identity")
                            ...
                            .attr("type", "text")
                            .attr("name", format!("{} Group Chat", state.server_name())),
                    );
                for feature in [
    ...
    if matches!(
        ...
        let (affiliation_changed, previous_affiliation, affiliation, notice_nick) = match action {
            ...
            FederatedRegistrationAction::Register(nick) => {
                ...
                let mut occupants = state.federated_muc_global_occupants(&room_jid).await?;
                for (_, occupant) in state.muc_occupants_for(&room_jid) {
                    occupants.insert(
                        occupant.nick.clone(),
    ...
    if to.contains('/') {
        ...
        let Some(target) = state.local_muc_occupant_by_nick(&room_jid, target_nick) else {
            return Ok(federated_error(
                &request.stanza,
                from,
                "cancel",
...
async fn federated_muc_owner(
    ...
    match action {
        FederatedOwnerAction::Cancel => {
            if room.is_locked() {
                ...
                for (_, occupant) in state.muc_occupants_for(&requester.room_jid) {
                    let Some(occupant) = state.remove_local_muc_occupant_exact((&occupant).into())
                    else {
                        continue;
                    };
                    let serializable = SerializableMucOccupant::from(&occupant);
        ...
        FederatedOwnerAction::Configure(config) => {
            ...
            {
                let global = state
                    .federated_muc_global_occupants(&requester.room_jid)
                    .await?;
                for (nick, raw) in global {
                    ...
                    let locally_owned =
                        state.local_muc_occupant_exact((&occupant).into()).is_some();
                    if locally_owned {
                        continue;
                    }
                    ...
                    {
                        let _ = state
                            .federated_muc_change_occupant_policy(
                                &requester.room_jid,
                                &occupant,
                                desired_role,
...
async fn publish_occupant_change(
    ...
    if let Some(status) = removal_status {
        ...
        state
            .federated_muc_evict_occupant(&serializable, status, Some(actor_nick), change.reason)
            .await?;
        if state.muc_occupants_for(&target.room_jid).is_empty() {
            state.federated_muc_leave_room(&target.room_jid).await?;
        }
        state
            .federated_muc_publish_removal_presence(
                &target.room_jid,
        ...
        let json = serde_json::to_string(&serializable)?;
        state
            .federated_muc_register_occupant(&target.room_jid, &target.nick, &json)
            .await?;
        state
            .federated_muc_publish_presence(&target.room_jid, &serializable, false, false, None)
            .await?;
        for (_, recipient) in state.muc_occupants_for(&target.room_jid) {
...
async fn federated_muc_admin_set(
    ...
    for item in &items {
        ...
        } else if let (Some(target_nick), Some(role)) = (item.nick.as_deref(), item.role.as_deref())
        {
            ...
            let target = if state.federated_muc_pg_authority_enabled() {
                ...
                state
                    .local_muc_occupant_by_nick(&requester.room_jid, &target_nick)
                    .as_ref()
                    .map(SerializableMucOccupant::from)
    ...
    for item in items {
            ...
            for remote in remote_present {
                ...
                if remove_from_room {
                    ...
                    let updated = match state
                        .federated_muc_change_occupant_affiliation(
                            &requester.room_jid,
                            &remote,
                            affiliation,
                            role,
        ...
        } else if let (Some(target_nick), Some(role)) = (item.nick.as_deref(), item.role.as_deref())
        {
            ...
            let local_target = state.local_muc_occupant_by_nick(&requester.room_jid, &target_nick);
            if local_target.is_none() {
                ...
                if role == "none" {
                    ...
                    let updated = match state
                        .federated_muc_change_occupant_role(&requester.room_jid, &target, role)
                        .await?
                    {
                        crate::cluster::MucRoleChange::Changed(updated) => *updated,
...
async fn federated_muc_moderate(
    ...
    match state
        .federated_muc_publish_room_message(&moderator.room_jid, &notice, None)
        .await
    {
        Ok(()) => {}
        Err(_) => record_federated_muc_post_commit_failure(
...
pub(crate) async fn federated_muc_connection_closed(
    ...
    let departed =
        state.federated_muc_occupants_for_closed_connection(authenticated_domain, connection_id);
    for occupant in departed {
        unregister_remote_occupant(state, occupant, Some(333)).await?;
    }
    Ok(())
}
