...
class PackageTests(unittest.TestCase):
    ...
    def test_draft_notes_bind_ci_and_only_claim_completion_after_download_validation(self):
        ...
        self.assertNotIn('Package and fresh-download checks passed.', output.read_text())
        NOTES.render(q, e, output, True)
        self.assertIn('Package and fresh-download checks passed.', output.read_text())
        self.assertNotIn('Draft verification is still running', output.read_text())
        self.assertTrue(output.read_text().startswith('Northstar is developed collaboratively by humans and AI.'))
