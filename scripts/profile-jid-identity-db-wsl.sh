...
test_schema="northstar_profile_jid_it_$random_suffix"

if [[ "$test_database" != "xmpp_test" ]]; then
  echo "profile JID tests are restricted to the dedicated xmpp_test database" >&2
  exit 2
fi
if [[ ! "$test_schema" =~ ^northstar_profile_jid_it_[a-f0-9]{32}$ ]] ||
   (( ${#test_schema} > 63 )); then
  echo "refusing an unsafe profile JID test schema name" >&2
  exit 2
fi
...
test_name="db::profile_identity::tests::postgres_profile_item_ids_fail_closed_and_restart_idempotently"
...
if ! grep -Eq 'test result: ok\. 1 passed; 0 failed' <<<"$output"; then
  echo "expected exactly one ignored profile JID database test to execute" >&2
  exit 1
fi
...
if [[ "$remaining" != "0" ]]; then
  echo "isolated profile JID schema was not removed" >&2
  exit 1
fi
trap - EXIT
