...
| `account-revocations` | `main`, cluster only | critical / continuous | 10 s | immediate | consume committed PostgreSQL account fences and acknowledge exact revisions; wake on NOTIFY with a 1 s fallback | database errors fence local routes; repeated errors, task exit or watchdog expiry shut down the node |
