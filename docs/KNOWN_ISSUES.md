...
| PROFILE-REVOCATION | 本地 CRL 與自有服務憑證的可選 OCSP stapling 已實作；尚無線上取得回應或對端 S2S staple 驗證 | 產品範圍＋網路安全取捨 | **部分實作，仍需環境驗證** | 操作者可提供 DER OCSP 回應；載入時核驗回應者授權、葉證書、`good` 狀態與七日內的有效區間，過期後拒絕新握手，無效 reload 保留舊快照。C2S 客戶端憑證與聯邦對端仍依本地 CRL 政策；DANE-EE 不套用 CA 撤銷 | 在目標憑證與客戶端矩陣驗證輪換、失效與兩版 TLS；若增加線上來源，先定義來源白名單、地址及重定向限制、快取、超時和失敗政策。未完成前不宣稱線上 OCSP/CRL/AIA 支援 |
