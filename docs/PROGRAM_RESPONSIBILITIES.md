...
| `runtime-control-refresh` | `AppState` runtime-control startup | critical / continuous | 5 s | immediate | hold the single connection reserved during startup; sequentially refresh durable federation rules and security-relevant administration settings each second, and observe XEP-0133 service control every 500 ms when enabled | pooling a runtime control read after activation, splitting control-plane reads into competing workers, silently retaining a superseded security setting, continuing with stale allow/deny authority, or keeping listeners available after committed shutdown control changes |
...
| runtime control pool | `northstar_runtime` | exactly 1 reserved connection, 500 ms acquire bound | durable administration-settings refresh; XEP-0133 restart/shutdown watcher when enabled | separate from traffic and command pools; an exhausted workload pool cannot hide committed safety configuration |
